...
Extra: If you want to capture network attributes from a statnet object in variables, many of them are programmatically accessible through the `gal` element. For example, the vertex count and count of edges can be accessed like this:
...
To see all of the available attributes in a statnet object, reference the `gal` element in a terminal window. Use `$gal` to show the attributes of the `karate.stat` object below. 
