...
Note: There are [16 possible classes](https://www.researchgate.net/figure/The-sixteen-isomorphism-classes-of-triad-census-for-directed-networks-D-stands-for_fig2_328894419) of directed triads (see Wasserman and Faust, fig. 6.7), only four of which are possible in undirected graphs. igraph considers this context undefined, and will output a warning message if `triad.census` is called on an undirected graph. Both igraph and statnet will only output the results for these four undirected permutations in undirected graphs, as the others cannot be determined.
