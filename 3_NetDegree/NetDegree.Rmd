...
The `centr_degree` function returns an object with several components, we are interested in the `centralization` component. It requires specification of options as in the `igraph::degree` command used above. This code is not evaluated to avoid repetition.
...
Using the `slice(t)` command tells R to return only the `t`th row of the data object. In this case, we tell R to return rows 1, 2 and 3 (1:3) of the data object arranging in descending order by scores - or the top three scores.
