...
In statnet, we use `evcent()` to calculate node-level eigenvector centrality scores in statnet, and the familiar `centralization()` score with the function "evcent" to calculate the network-level eigenvector centralization. As above, we are using the `head` command to just view the first 6 node-level eigenvector centrality scores.
